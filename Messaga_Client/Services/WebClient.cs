...
namespace Messaga_Client.Services
{
    internal class WebClient : IWebClient
    {
        ...
        public Func<Message, Task> MessageRecieveAsync { get; set; }//внешний делегат для обработки входящего сообщения
        ...
        public async Task<bool> ConnectToMessageHub()
        {
            try
            {
                _messHabConnection = new HubConnectionBuilder()
                    .WithUrl($"{_serverUrl}/chat", options =>
                    {
        ...
        public async Task<bool> TrySendMessageToHub(Message message)
        {
            try
            {
                if (_messHabConnection != null && _messHabConnection.State == HubConnectionState.Connected)
                {
                    await _messHabConnection.InvokeAsync("SendMessageToClient", message);//отправка конкретному клиенту(через сервер)
                    return true;
                }
            }
            catch(Exception ex) { }
