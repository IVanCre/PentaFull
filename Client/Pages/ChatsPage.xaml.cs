            ...
            MainThread.BeginInvokeOnMainThread(() =>
            {
                ...
                foreach (var chatInfo in findedChats)
                {
                    ...
                    if (identityContact != null)
                        ChatsList.Add(new ChatInfo(chatInfo.ID, identityContact.UserName, chatInfo.ChatType,chatInfo.HaveUnreadedMessages));//прописываем им€ юзера из контакта
                    else
...
button.BackgroundColor = backClr;
        ...
        private async void ChatCreated(int chatID, string chatName)//обработка ответа на создание группового чата 
        {
            ...
            if (findedContact != null)
                createdChatInfo = new ChatInfo(chatID, findedContact.UserName, ChatType.Private,false);//сохран€ем с указанным именем(т.к. есть аналогичный контакт)
            else
            {
                var findedChat = await _clientFacade.GetChatByID(chatID);
                if(findedChat!=null)
                    createdChatInfo = new ChatInfo(chatID, chatName, findedChat.ChatType,false);//сохран€ем с исходным именем
            }
