    ...
    public class UsersConnectionObserver: IUsersConnectionObserver
    {
        ...
        private void ProcessUserConnectionState(int userID, bool state)
        {
            ...
            foreach(var item in finded)
                _ = _notifier.SendToUser(MessageFactory.UserInSystemState(item.Value.Item1,item.Value.Item2,item.Key, state));
        }
        ...
        public void AddToObserve(int chatID, int recieverID, int userIDToObserve)
        {
            if (!_observablConn.ContainsKey(recieverID))
            {
                ...
                _notifier.SendToUser(MessageFactory.UserInSystemState(chatID,userIDToObserve, recieverID,!string.IsNullOrEmpty(findedConnection)));
            }
        }
